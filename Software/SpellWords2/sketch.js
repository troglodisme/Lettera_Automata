  ...
  if (printing && currentY < lines.length) {
    ...
    if (canPrint && timer <= 0) {
      if (currentX < lines[currentY].length) { // Move in the same line
        ...
        currentX = 0;
        c = lines[currentY][currentX];
        serial.write("~" + lines[currentY].split(" ")[currentN]); // Send a warning as the next data will be a string then send the current word
      ...
      if (printing && (c == " " || (currentX == 0 && currentY == 0))) {
        if(c == " ")
          currentN++; // Since a word is .split(" "), whenever a space is seen go to the next word, note that a space is also a word in that context
        serial.write("~" + lines[currentY].split(" ")[currentN]); // Send a warning as the next data will be a string then send the current word
      }
